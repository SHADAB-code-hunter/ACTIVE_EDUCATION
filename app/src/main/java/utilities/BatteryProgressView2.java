...
public class BatteryProgressView2 extends View {
    ...
    private int progressColor=0xFF19A1C4;
    ...
    public BatteryProgressView2(Context context) {
        super(context);
        init(context,null);
    }

    public BatteryProgressView2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);
    }
    ...
    public void setProgress(int progress, int circle_progress_color1) {
        lastProgress=this.progress;
        this.progressColor=circle_progress_color1;
        setProgressColor(circle_progress_color1);
        this.progress = progress;
        post(new Runnable() {
            @Override
            public void run() {
                float incr=360/maxProgress;
                Log.e("pogress","last:"+lastProgress+",progress:"+ BatteryProgressView2.this.progress);
                if(lastProgress< BatteryProgressView2.this.progress) {
                    Log.e("first",lastProgress+" to "+ (incr * (BatteryProgressView2.this.progress))+":"+lastProgress);
                    animator = ValueAnimator.ofFloat(incr*lastProgress, incr * (BatteryProgressView2.this.progress));
                    animator.setDuration(800);
                    animator.addUpdateListener(animatorUpdateListener);
                    animator.setInterpolator(new DecelerateInterpolator());
                    animator.start();
                }else {
                    Log.e("second",lastProgress+" to "+ (incr * (BatteryProgressView2.this.progress))+":"+lastProgress);
                    animator = ValueAnimator.ofFloat((incr*lastProgress), incr * (BatteryProgressView2.this.progress));
                    animator.setDuration(800);
                    animator.addUpdateListener(animatorUpdateListener);
                    animator.setInterpolator(new DecelerateInterpolator());
