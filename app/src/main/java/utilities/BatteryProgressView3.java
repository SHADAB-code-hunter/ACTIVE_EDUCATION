...
public class BatteryProgressView3 extends View {
    ...
    private int progressColor=0xFFFF4A8E;
    ...
    public BatteryProgressView3(Context context) {
        super(context);
        init(context,null);
    }

    public BatteryProgressView3(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);
    }
    ...
    public void setProgress(int progress, int circle_progress_color1) {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                Log.e("pogress","last:"+lastProgress+",progress:"+ BatteryProgressView3.this.progress);
                if(lastProgress< BatteryProgressView3.this.progress) {
                    Log.e("first",lastProgress+" to "+ (incr * (BatteryProgressView3.this.progress))+":"+lastProgress);
                    animator = ValueAnimator.ofFloat(incr*lastProgress, incr * (BatteryProgressView3.this.progress));
                    animator.setDuration(800);
                    animator.addUpdateListener(animatorUpdateListener);
                    animator.setInterpolator(new DecelerateInterpolator());
                    animator.start();
                }else {
                    Log.e("second",lastProgress+" to "+ (incr * (BatteryProgressView3.this.progress))+":"+lastProgress);
                    animator = ValueAnimator.ofFloat((incr*lastProgress), incr * (BatteryProgressView3.this.progress));
                    animator.setDuration(800);
                    animator.addUpdateListener(animatorUpdateListener);
                    animator.setInterpolator(new DecelerateInterpolator());
