...
class RadioButtonDrawable extends Drawable implements Animatable {
    ...
    private void drawUnchecked(Canvas canvas){
        ...
        mPaint.setStrokeWidth(mStrokeSize);
        mPaint.setStyle(Paint.Style.STROKE);
        Log.d("canvas",cx+" "+cy+" "+mRadius+" "+mPaint);
       // canvas.drawCircle(cx, cy, mRadius, mPaint);
        if(!explosionAnimationIsRunning){
            explosionAnimationIsRunning = true ;
            Log.d("bounds",""+mExplodeCounts+" "+mRadius+" "+mInnerRadius);
            ExplosionAnimator animator = new ExplosionAnimator(this,getBounds(),mCurColor,mExplodeCounts,200,mInnerRadius);
    ...
    protected boolean onStateChange(int[] state) {
      ...
      /*  if(mChecked != checked){
            ...
            if(!mInEditMode && mAnimEnable) {
              //  Log.d("gfhg",mInEditMode+" "+mAnimEnable+" "+explosionAnimationIsRunning+" "+mChecked);
                start();
            }
